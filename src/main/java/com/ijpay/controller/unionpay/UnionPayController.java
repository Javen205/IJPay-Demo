...
package com.ijpay.controller.unionpay;
...
/**
 * @author Javen 2017年10月5日
 */
public class UnionPayController extends Controller {
	static Log log = Log.getLog(WxPayController.class);
	private AjaxResult ajax = new AjaxResult();

	public void index() {
		log.info("欢迎使用IJPay银联支付	- by Javen");
		renderText("欢迎使用IJPay银联支付	- by Javen");
	}
...
//					getCustomerInfo(customerInfoMap,null,DemoBase.encoding);
	...
	public void backRcvResponse() {
		...
		if (!AcpService.validate(reqParam, encoding)) {
			log.info("后台验证签名结果[失败].");
			// 验签失败，需解决验签问题

		} else {
			log.info("后台验证签名结果[成功].");
